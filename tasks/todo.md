...
- [ ] Step 1: Create `.github/workflows/ci.yml` — runs on `pull_request` and `push` to `main`
- [ ] Step 2: Job matrix with all 5 test suites running in parallel (ubuntu-latest, bash + python3 pre-installed)
- [ ] Step 3: Add JSON validation step (`find . -name "*.json" | python3 validate`)
- [ ] Step 4: Add shell syntax validation step (`bash -n` on all 4 scripts)
- [ ] Step 5: Add sanitization check step (grep for personal data patterns)
- [ ] Step 6: Add plugin manifest validation step (no invalid commands/skills fields)
- [ ] Step 7: Test the workflow by pushing the feature branch and verifying Actions run

### Part 2 — GitHub Release (v1.0.0)
- [ ] Step 8: Create `v1.0.0` GitHub Release via `gh release create` with changelog summary
- [ ] Step 9: Update README.md with release badge

## Verification
- [ ] CI workflow triggers on PR creation
- [ ] All 5 test suites pass in CI (146/146)
- [ ] All validation checks pass (JSON, shell, sanitization, manifests)
- [ ] GitHub Release v1.0.0 exists with proper description
